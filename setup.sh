...
function updateenv() {
    ...
    if [ $? -ne 0 ]; then
        echo "Failed installing 3commas-ftx-triggers"
        exit 1
    fi
...
function install() {
    ...
    reset
    echo "-----------------------------------------"
    echo "Installation of 3commas-ftx-trigger done!"
    echo "-----------------------------------------"
    ...
    if [ ! -f "lbotid_list.txt" ]; then
        echo "---------------------------"
        echo "Deploy the bots on 3Commas."
        echo "---------------------------"
        echo "Create the bots on 3Commas by executing 'Py3x_create.py'."
    else
        echo "Enable the bots on 3Commas by executing 'Py3x_update.py'."
    fi
    echo
    echo "Run the 'Py3c_triggers.py' script file to start the bot trigger script."
}


function help() {
    echo "usage:"
    echo "	-i,--install    Install 3commas-ftx-triggers from scratch"
